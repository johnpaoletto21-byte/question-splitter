...
import {
  ...
  bboxToPixelRect,
  BboxInvalidError,
} from '../crop-engine/index';
import type {
  CropEngineTargetResult,
  CropRegionPixels,
  PixelRect,
} from '../crop-engine/types';

// Re-export for callers that import from the orchestrator barrel.
...
export type CropStepTargetResult =
  | {
      status: 'ok';
      targetId: string;
      regions: Array<CropRegionPixels & { cropFilePath: string }>;
    }
 ...
 */
export async function runCropStep(
  runId: string,
  localizedTargets: LocalizationResult[],
  pages: PreparedPageImage[],
  cropExecutor: CropExecutor,
): Promise<CropStepTargetResult[]> {
  const results: CropStepTargetResult[] = [];

  for (const localized of localizedTargets) {
    const { target_id: targetId, regions } = localized;

    try {
      const cropRegions: Array<CropRegionPixels & { cropFilePath: string }> = [];
      ...
      for (const region of regions) {
        ...
        validateBbox(bbox_1000, targetId);
        ...
        if (!page) {
          ...
          throw new BboxInvalidError(
            targetId,
            bbox_1000,
            `no PreparedPageImage found for page_number ${page_number}`,
          );
        }

        // Convert normalized bbox to pixel coordinates using rendered dimensions.
        const pixelRect = bboxToPixelRect(bbox_1000, page.image_width, page.image_height);
