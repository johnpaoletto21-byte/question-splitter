 ...
 */
function deriveSourceId(inputOrder: number, filePath: string): string {
 ...
 */
export function bootstrapRun(request: RunRequest): RunContext {
  const { pdfFilePaths, config, runLabel } = request;

  if (!Array.isArray(pdfFilePaths) || pdfFilePaths.length === 0) {
    throw new RunBootstrapError(
      'pdfFilePaths must be a non-empty array. Provide at least one PDF file path.'
    );
  }
  ...
  for (let i = 0; i < pdfFilePaths.length; i++) {
    ...
    if (typeof p !== 'string' || p.trim() === '') {
      throw new RunBootstrapError(
        `pdfFilePaths[${i}] must be a non-empty string. Received: ${JSON.stringify(p)}`
      );
    }
  }

  // Preserve input order exactly — do not sort
  const sources: PdfSource[] = pdfFilePaths.map((filePath, index) => ({
  ...
  return {
    ...
    promptSnapshot: request.promptSnapshot ?? capturePromptSnapshot(),
