...
import { composeOutput } from '../output-composer/composer';
import type { ImageStackerFn } from '../output-composer/composer';
import { CompositionError } from '../output-composer/types';
import type { FinalResultRow } from '../result-model/types';

// Re-export so callers import from the orchestrator barrel.
...
export async function runCompositionStep(
  runId: string,
  cropResults: CropStepTargetResult[],
  ...
  const sourcePagesMap = new Map<string, number[]>(
    localizedTargets.map((lt) => [
      lt.target_id,
  ...
  for (const cropResult of cropResults) {
    ...
    try {
      const composed = await composeOutput(
        {
          targetId,
          sourcePages,
