...
describe('V1_ACTIVE_PROFILE — centralized V1 policy constants', () => {
  ...
  it('max_regions_per_target is 10', () => {
    expect(V1_ACTIVE_PROFILE.max_regions_per_target).toBe(10);
  });
...
describe('validateCropTargetProfile — max_regions_per_target violations (INV-3)', () => {
  it('throws ProfileValidationError for max_regions_per_target = 11 (exceeds V1 limit)', () => {
    const profile: CropTargetProfile = {
      ...V1_ACTIVE_PROFILE,
      max_regions_per_target: 11,
    };
    expect(() => validateCropTargetProfile(profile)).toThrow(ProfileValidationError);
  });
  ...
  it('error message mentions the valid range', () => {
    ...
    try {
      ...
      expect((err as ProfileValidationError).message).toMatch(/\[1, 10\]/);
    }
  });
});
