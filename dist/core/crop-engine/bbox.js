...
exports.bboxToPixelRect = bboxToPixelRect;
 ...
 */
function validateBbox(bbox, targetId) {
    const labels = ['y_min', 'x_min', 'y_max', 'x_max'];
    for (let i = 0; i < 4; i++) {
        const v = bbox[i];
        if (!Number.isInteger(v)) {
            throw new types_1.BboxInvalidError(targetId, bbox, `${labels[i]} (${v}) is not an integer`);
        }
        if (v < 0 || v > 1000) {
            throw new types_1.BboxInvalidError(targetId, bbox, `${labels[i]} (${v}) is out of range [0, 1000]`);
        }
    }
    const [yMin, xMin, yMax, xMax] = bbox;
    if (yMin >= yMax) {
        throw new types_1.BboxInvalidError(targetId, bbox, `y_min (${yMin}) must be less than y_max (${yMax}) — inverted or zero-height`);
    }
    if (xMin >= xMax) {
        throw new types_1.BboxInvalidError(targetId, bbox, `x_min (${xMin}) must be less than x_max (${xMax}) — inverted or zero-width`);
    }
}
// ---------------------------------------------------------------------------
// bboxToPixelRect
 ...
 */
function bboxToPixelRect(bbox, imageWidth, imageHeight) {
    const [yMin, xMin, yMax, xMax] = bbox;
    return {
