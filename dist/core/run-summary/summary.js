 ...
 */
function applyFinalResultsToSummary(state, rows) {
    // Build a target-id → index map for O(1) lookup.
    const indexMap = new Map(state.targets.map((t, i) => [t.target_id, i]));
    ...
    for (const row of rows) {
        ...
        if (idx === undefined) {
            throw new Error(`applyFinalResultsToSummary: target_id "${row.target_id}" not found in summary state. ` +
