 ...
 */
function renderBlendConfigInputs(
  defaults: BlendConfigDefaults,
...
export function renderHintFormHtml(input: {
  configReady: boolean;
...
function renderLogsBlock(record: LocalHintRunRecord): string {
...
export function renderHintStatusHtml(record: LocalHintRunRecord): string {
...
export function renderHintResultsHtml(
  record: LocalHintRunRecord,
  blendDefaults?: BlendConfigDefaults,
): string {
  ...
  const methodLabel =
    result.method === 'overlay' ? 'Canvas Overlay' :
    result.method === 'image-gen' ? 'Image Generation' :
    result.method === 'blend' ? 'Blend' : result.method;

  const blendRetryBlock =
    result.method === 'blend' && blendDefaults
      ? `
...
export function renderHintAllResultsHtml(record: LocalHintRunRecord): string {
  ...
  const successCount = methods.filter((m) => allResults[m.key as keyof typeof allResults]).length;
  ...
  for (const m of methods) {
    const result = allResults[m.key as keyof typeof allResults];
    if (result) {
      panels.push(`
        <div class="panel">
...
export function renderHintErrorHtml(title: string, message: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
